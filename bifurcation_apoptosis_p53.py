...
def main():
    ...
    params[0] = 2e5     # set p53_killer to 2e5
    ...
    p53_killer = dst.Par(0, 'p53_killer')
    ...
    DSargs.ics = dst.args(                              # initial states
        Bax_mRNA=eq1[0],
        Bax=eq1[1],
        BclXL=eq1[2],
        Bax_BclXL_cx=eq1[3],
    ...
    PCargs.freepars = ['p53_killer']                    # set free parameter
    PCargs.MaxNumPoints = 925
    ...
    # compute a the first equilibrium point curve
    print("Numerical Continuation 1...")
    PC.newCurve(PCargs)
    PC['EQ1'].forward()

    # draw first bifurcation curve (p53_arrester vs. CyclinE)
    PC.display(['p53_killer', 'Casp'], stability=True, figure=fig)
    ...
    # change the initial value of the free parameter to 2e5 (corresponds to the steady state above)
    ode.set(pars={'p53_killer': 2e5})

    # define numerical continuation problem
    PC = dst.ContClass(ode)
    PCargs = dst.args(name='EQ1', type='EP-C')
    PCargs.freepars = ['p53_killer']
    PCargs.MaxNumPoints = 910
    PCargs.MaxStepSize = 250
    PCargs.MinStepSize = 1
    PCargs.StepSize = 25
    ...
    # edit figure
    plt.xlabel(r'$\mathregular{p53_{KILLER}}$')
    plt.ylabel('Caspase')
    plt.title(r'$\mathregular{Akt = 0}$', fontsize=15)
    plt.yscale('log')
    plt.ylim([2e-2, 3e5])
    plt.xticks(ticks=[0, 1e5, 2e5])
    plt.tick_params(labelsize=10)
    fig.tight_layout()
    plt.show()
    if save:
        fig.savefig('bifurcation_apoptosis_p53.png', dpi=300)
